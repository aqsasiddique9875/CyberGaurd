...
def make_downloadable_json(obj, name="result.json"):
    ...
    return st.download_button("⬇ Download JSON", data=b, file_name=name, mime="application/json")
...
def make_downloadable_report(result, name_prefix="report"):
    ...
    return st.download_button("⬇ Download Report (TXT)", data=data, file_name=f"{name_prefix}.txt", mime="text/plain")
...
# ----------------------------
ADMIN_PASSWORD = st.secrets.get("ADMIN_PASSWORD", None) if "secrets" in st._dict_ else None
